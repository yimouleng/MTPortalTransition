...
- (void) dismissPortalTransitionViewController: (UIViewController *)viewControllerToPresent completion:(void (^)(void))completion
{
    //Get up most parentViewController
    UIViewController *fromViewController = viewControllerToPresent;
    while (nil != [fromViewController parentViewController]) {
        fromViewController = [fromViewController parentViewController];
    }
    CYPortalTransitionContext *transitionContext = [[CYPortalTransitionContext alloc] initWithFromViewController:fromViewController toViewController:self];
    transitionContext.animated = YES;
    transitionContext.interactive = NO;
    transitionContext.completionBlock = ^(BOOL didCompleted){
        if (didCompleted) {
