...
- (void)setup{
    ...
    _portalAnimationDuration = 1.0f;
    _scaleAnimationDuration = 0.5f;
    _delay = 0.2f;
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(getImg:) name:@"leftAndRight" object:nil];
}
- (void)getImg:(NSNotification *)info{
    
    self.left = info.userInfo[@"1"];
    self.right = info.userInfo[@"2"];
}
...
- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext;
{
    ...
    UIView *fromView = toViewController.view;
    UIView *toView = fromViewController.view;
    ...
    if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)])
        UIGraphicsBeginImageContextWithOptions(toView.window.bounds.size, NO, screenScale);
    else
        UIGraphicsBeginImageContext(toView.window.bounds.size);
    
    [toView.window.layer renderInContext:UIGraphicsGetCurrentContext()];
    ...
    leftImageView.image = self.left;
    rightImageView.image = self.right;
...
//    toView.transform = CGAffineTransformMakeScale(_targetViewScale,_targetViewScale);
    ...
    reflectionView.frame = CGRectMake(0, 0, width, height);
...
//    reflectionView.frame = reflectionViewFrame;
   ...
   // reflectionView.alpha = 0.4;
    ...
    leftImageView.frame = CGRectMake(-width/2, 0, width/2, height);
    rightImageView.frame = CGRectMake(width, 0, width/2, height);
    [UIView animateWithDuration:_scaleAnimationDuration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
//        toView.alpha = 1;
//        reflectionView.alpha = 1;
    toView.transform = CGAffineTransformMakeScale(0.95, 0.95);
       //reflectionView.transform = CGAffineTransformMakeScale(0.5, 0.5);
       // toView.transform = CGAffineTransformMakeScale(1, 1);
        leftImageView.alpha = 1;
        rightImageView.alpha = 1;
        
        leftImageView.frame = CGRectMake(0, 0, width/2, height);
        rightImageView.frame = CGRectMake(width/2, 0, width/2, height);
...
//        }
      ...
      //  if (_portalTransitionStyle == CYPortalTransitionStylePresent) {
            ...
            [fromViewController dismissViewControllerAnimated:toViewController completion:nil];
       // }
